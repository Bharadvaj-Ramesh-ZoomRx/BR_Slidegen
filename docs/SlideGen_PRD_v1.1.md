...
* **Ownership** (Q3 Plan): workflows, not individual skills
  ...
  * Bharadvaj owns the `wave-refresh-workflow` end-to-end
...
### 6.8 Data Lineage via Galen-PowerPoint Connector Tags

The existing **Galen-PowerPoint Synapse Connector** already stamps Synapse configuration onto shapes in decks authored through the add-in. SlideGen **leverages these tags as the preferred source of data lineage** — but does not require them, and does NOT trust them blindly. `deck-reader` is two-tier with explicit tag health checks:
...
### 6.9 Headlines must refresh with data
...
|-|-|-|
|**Apr 15 — DONE**|Deck analysis + `pptx_utils` foundation|32 decks analyzed; 18 BRAND entries, 11 LAYOUTS, 10 CHART_PATTERNS, 9 new `lxml_helpers` landed|
...
|**End of Apr**|Rendering fidelity complete|Visual regression passing against coordinate clusters on top-6 chart patterns × representative brands. Line/doughnut Repair bug closed. Connector-tag integration tested against a real tagged PET deck.|
...
|**End of May**|Primary demo: refresh deck (PET)|Full `refresh-deck-workflow` on a real PET brand, both Connector-tagged and untagged refresh paths demonstrable. Headline regen working (§6.9).|
